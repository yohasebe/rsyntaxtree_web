...
* 入力中に，誤りのある行に印が付くようになりました．原因と直し方が一行で表示されます [2026年8月]
* `\[cons\]` のようにエスケープした括弧が，エディタで構造として扱われなくなりました [2026年8月]
* 線幅を文字サイズに対する比で指定：0.5 から 3.0 まで 0.5 刻み．どの文字サイズでも文字と線の釣り合いが変わりません [2026年8月]
* 入力が受け付けられなかったとき，直し方が一行で示されるようになりました [2026年8月]
* ラベル全体を行列にできます．上にラベルを置く必要はありません：`[#(HEAD\tnoun#)]` [2026年8月]
...
* `縦間隔` オプション（当時の名称はコネクタの高さ） [2018年3月]
