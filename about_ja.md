...
* 多言語描画の改善：あらゆる文字体系（クメール文字，タイ文字，アラビア文字，ビルマ文字，デーヴァナーガリーなど）のラベルを，描画と同じエンジンで計測し正しく中央配置 [2026年8月]
* 表示設定を次回訪問時にも記憶するようになりました（ブラウザ内にローカル保存。入力テキストは保存されません）[2026年8月]
* `%`構文による領域シェード：ノードが支配する部分木全体を網掛け [2026年6月]
* ラベル中の直線アポストロフィを，タイポグラフィ的な（曲線の）アポストロフィとして描画 [2026年6月]
* 左から右へのツリーレイアウト（`方向`オプション：上から下 / 左から右）[2026年4月]
* `@color:`構文によるノード別カラー指定（色名およびHEXカラー）[2026年1月]
...
* 下付き文字（`_text_`）と上付き文字（`__text__`）の指定が可能に（例：`DP_i_` → DP<sub>i</sub>、`T__0__` → T<sup>0</sup>）[2021年12月]
* 終端ノード内で改行文字`\n`が使用可能に [2021年12月]
